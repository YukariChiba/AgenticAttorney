...
辩论引擎，基于 AutoGen 实现逆转裁判风格的多 agent 辩论。
...
起始准备环节是让双方 Agent 先收集一些资料，这样可以降低后续辩论中收集资料的 token 消耗，使辩论更加流畅。
...
每个参与辩论的 agent 均拥有自己的备忘录 memory，可随时写入或阅读，以便使 agent 回忆起重要的事实和较远的上下文。
...
- `summary_start`: 上下文摘要起始点
- `summary_end`: 上下文摘要结束点
